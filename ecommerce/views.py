...
def index(request):
    categories = Category.objects.prefetch_related(
        Prefetch('product_set', queryset=Product.objects.order_by('-id')[:20], to_attr='top_products')
    )
...
class ProductDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['avg_rating'] = round(reviews.aggregate(Avg('rating'))['rating__avg'] or 0, 1)
...
class ProductListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.is_authenticated:
            saved_product_ids = Saved.objects.filter(user=self.request.user).values_list('product_id', flat=True)
...
def add_to_cart(request, product_id):
    ...
    return JsonResponse({
        ...
        'redirect_url': reverse('view_cart')  # Correct dynamic URLredirect_url': '/cart/'  # or use reverse('view_cart')
    })
...
def quick_view_product(request, product_id):
    ...
    html_content = render_to_string('ecommerce/product_quick_view.html', context, request=request)
...
def saved_items_view(request):
    ...
    return render(request, 'ecommerce/saved_items.html', {
